...
1. **개발 서버 구동 방식**: 프론트엔드(Vercel)와 백엔드(Render.com)를 물리적으로 완전히 분리하는 **분리형 아키텍처(Decoupled Architecture)**를 채택합니다.
   - **이유:** 프론트엔드는 글로벌 CDN을 통한 정적 서빙과 빠른 UI 렌더링에 집중하고, 백엔드는 긴 Time-to-First-Byte(TTFB)를 요구하는 AI SSE 스트리밍 통신에 집중하도록 부하를 분산하기 위함입니다.
...
### Deployment Strategy (인프라 배포 전략)

* **Frontend (Vercel):** `fo/` 폴더를 루트로 지정하여 Vite 빌드 실행. `VITE_API_URL` 환경 변수를 통해 백엔드 엔드포인트를 런타임 주입.
* **Backend (Render):** `api/` 폴더를 루트로 지정하여 Node 컨테이너 실행. LLM 통신용 API 키(`GEMINI_API_KEY`)를 환경 변수로 은닉 관리.
